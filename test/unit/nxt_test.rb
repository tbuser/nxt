...
class NXTTest < Test::Unit::TestCase
  ...
  def test_get_and_set_output
    assert @@nxt.set_output_state(NXT::MOTOR_A,100,NXT::MOTORON,NXT::REGULATION_MODE_MOTOR_SPEED,100,NXT::MOTOR_RUN_STATE_RUNNING,0)
    state = @@nxt.get_output_state(NXT::MOTOR_A)
    assert_equal 100, state[:power]

    sleep(1)

    assert @@nxt.set_output_state(NXT::MOTOR_A,0,NXT::BRAKE,NXT::REGULATION_MODE_MOTOR_SPEED,0,NXT::MOTOR_RUN_STATE_RAMPDOWN,0)
    state = @@nxt.get_output_state(NXT::MOTOR_A)
    assert_equal 0, state[:power]
  end

  def test_get_and_set_input
    assert @@nxt.set_input_mode(NXT::SENSOR_1,NXT::SWITCH,NXT::RAWMODE)
    values = @@nxt.get_input_values(NXT::SENSOR_1)
    assert_equal 0, values[:mode]

    assert @@nxt.set_input_mode(NXT::SENSOR_1,NXT::SWITCH,NXT::BOOLEANMODE)
    values = @@nxt.get_input_values(NXT::SENSOR_1)
    assert_equal 32, values[:mode]
  end

  def test_reset_input_scaled_value
    assert @@nxt.reset_input_scaled_value(NXT::SENSOR_1)
  end
  ...
  def test_reset_motor_position
    assert @@nxt.reset_motor_position(NXT::MOTOR_A)
  end
